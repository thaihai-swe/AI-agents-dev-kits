# Documentation

This documentation reflects the current 10-agent workflow defined in [`.github/agents/`](../.github/agents/).

## Start Here

- [guides/getting-started.md](guides/getting-started.md)
- [workflows/README.md](workflows/README.md)
- [reference/commands.md](reference/commands.md)

## Core Model

The kit separates durable repository memory from feature delivery artifacts.

Repo memory:

- `memories/repo/constitution.md`
- `memories/repo/project-knowledge-base.md`

`constitution.md` is normative policy. `project-knowledge-base.md` is descriptive context.

Feature artifacts:

- `artifacts/features/<slug>/analysis.md`
- `artifacts/features/<slug>/spec.md`
- `artifacts/features/<slug>/requirements-review.md`
- `artifacts/features/<slug>/design.md`
- `artifacts/features/<slug>/plan.md`
- `artifacts/features/<slug>/tasks.md`
- `artifacts/features/<slug>/review.md`

## Current Commands (5 Categories)

### Foundation
- `/constitution`
- `/project-knowledge-base`

### Discovery & Analysis
- `/analyze`

### Specification & Design
- `/spec-requirement`
- `/spec-review-requirements`
- `/spec-design`

### Planning & Delivery
- `/spec-plan`
- `/spec-tasks`

### Implementation & Quality
- `/spec-implement`
- `/spec-review`

Older docs that referenced `/discover`, `/architecture`, `/patterns`, or `/archive` were describing the retired model. See [Important Shift From The Older Model](agents/README.md#important-shift-from-the-older-model) for details.

## Documentation Map

### Guides

- [guides/README.md](guides/README.md)
- [guides/getting-started.md](guides/getting-started.md)

### Workflows

- [workflows/README.md](workflows/README.md)
- [WORKFLOW-REFERENCE.md](WORKFLOW-REFERENCE.md)
- [WORKFLOW-EXAMPLE.md](WORKFLOW-EXAMPLE.md)
- [how it work.md](how%20it%20work.md)

### Agents

- [agents/README.md](agents/README.md)
- [agents/AGENTS-REFERENCE.md](agents/AGENTS-REFERENCE.md)
- [agents/agent-mapping.md](agents/agent-mapping.md)
- [agents/details/specification-planning.md](agents/details/specification-planning.md)
- [agents/details/implementation-delivery.md](agents/details/implementation-delivery.md)
- [agents/details/discovery-learning.md](agents/details/discovery-learning.md)
- [agents/details/foundation-setup.md](agents/details/foundation-setup.md)
- [agents/details/quality-knowledge.md](agents/details/quality-knowledge.md)

### Reference

- [reference/README.md](reference/README.md)
- [reference/commands.md](reference/commands.md)
- [reference/faq.md](reference/faq.md)
- [reference/terminology.md](reference/terminology.md)

### Memory

- [memory-system/README.md](memory-system/README.md)

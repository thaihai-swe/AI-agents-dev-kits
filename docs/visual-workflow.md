...
### "Design didn't account for performance"
Path: `spec-implement` → loops back to `spec-design` → `spec-plan` / `spec-tasks` (add a profiling task) → continue implementation

```mermaid
flowchart TD
    Tasks[spec-tasks] --> Implement[spec-implement]
    Implement -->|performance issue found| Design[spec-design<br/>rethink approach]
    Design --> Plan[spec-plan<br/>add profiling task]
    Plan --> NewTasks[spec-tasks<br/>updated task list]
    NewTasks --> ContinueImpl[spec-implement<br/>optimization tasks]

    classDef loop fill:#ffd6d6,stroke:#c74545,color:#5c1414,stroke-width:2px;
    classDef process fill:#f3efff,stroke:#7b5cd6,color:#26174a,stroke-width:1px;

    class Tasks,Implement,Design,Plan,NewTasks,ContinueImpl process;
    class Implement,Design loop;
```\n\n**Example:** The real-time dashboard is too slow with 10k users. Rethink the data pipeline (websockets vs. polling, caching strategy), update the task list, implement optimizations.\n\n### \"Task decomposition was wrong"\nPath: `spec-implement` → `spec-review-implementation` → loops back to `spec-tasks` / `spec-plan` → continue or split remaining work\n\n```mermaid\nflowchart TD\n    Tasks[spec-tasks] --> Implement[spec-implement]\n    Implement --> Review[spec-review-implementation<br/>task check]\n    Review -->|task overlap or misjudged effort| Replan[spec-plan & spec-tasks<br/>rewrite task list]\n    Replan --> ContinueImpl[spec-implement<br/>revised tasks]\n    \n    classDef loop fill:#ffd6d6,stroke:#c74545,color:#5c1414,stroke-width:2px;\n    classDef process fill:#f3efff,stroke:#7b5cd6,color:#26174a,stroke-width:1px;\n    \n    class Tasks,Implement,Review,Replan,ContinueImpl process;\n    class Review,Replan loop;\n```\n\n**Example:** A task estimated at 2 days is actually 5 days. Review discovers overlapping concerns with another task. Rewrite the task list and adjust milestones.\n\n### \"Analysis was shallow"\nPath: `spec-requirement` → `spec-review-requirements` → loops back to `analyze` → refine `spec-requirement` → continue\n\n```mermaid\nflowchart TD\n    Analyze[analyze] --> Spec[spec-requirement]\n    Spec --> Review[spec-review-requirements]\n    Review -->|assumption was wrong| ReAnalyze[analyze<br/>deeper investigation]\n    ReAnalyze --> NewSpec[spec-requirement<br/>revised with correct data]\n    NewSpec --> NewReview[spec-review-requirements]\n    NewReview --> Plan[spec-plan]\n    \n    classDef loop fill:#ffd6d6,stroke:#c74545,color:#5c1414,stroke-width:2px;\n    classDef process fill:#f3efff,stroke:#7b5cd6,color:#26174a,stroke-width:1px;\n    \n    class Analyze,Spec,Review,ReAnalyze,NewSpec,NewReview,Plan process;\n    class Review,ReAnalyze loop;\n```\n\n**Example:** The feature spec assumes the codebase uses PostgreSQL, but analysis reveals it's MongoDB. Rebuild the spec with the correct data model assumptions.

### Testing Reveals Critical Issue

Path: `spec-testing-scenarios` → loops back to `spec-implement` → `spec-review-implementation` → `spec-testing-scenarios`

```mermaid
flowchart TD
    Review[spec-review-implementation<br/>approved] --> Testing[spec-testing-scenarios<br/>comprehensive testing]
    Testing -->|critical issue found| Implement[spec-implement<br/>fix the issue]
    Implement --> CodeReview[spec-review-implementation<br/>re-review fix]
    CodeReview --> ReTest[spec-testing-scenarios<br/>re-test]
    ReTest --> Done([Ready to merge])

    classDef loop fill:#ffd6d6,stroke:#c74545,color:#5c1414,stroke-width:2px;
    classDef process fill:#f3efff,stroke:#7b5cd6,color:#26174a,stroke-width:1px;
    classDef done fill:#eefbf2,stroke:#2f8f5b,color:#163a22,stroke-width:1px;

    class Review,Testing,Implement,CodeReview,ReTest process;
    class Testing,Implement loop;
    class Done done;
```\n\n**Example:** During integration testing, you discover that webhook duplicate IDs aren't actually being handled correctly due to a race condition in the database. Fix the race condition, re-review, and re-test.

## When to Loop vs. When to Push Through

**Loop back when:**
- You discover a fundamental mismatch (requirement wrong, design flawed, task boundary problems)
- Testing reveals a critical issue that affects the feature's core behavior
- A dependency or constraint changes that invalidates upstream work

**Push through when:**
- The issue is isolated to one task and doesn't affect the overall design
- It's a minor improvement that doesn't block the feature
- Testing reveals edge cases that can be fixed in-place without rethinking the design

The kit encourages looping because addressing root causes early prevents larger rewrites later.
